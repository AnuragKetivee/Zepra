...
#include "zeprascript/wasm/WasmSignalHandlers.h"
#include <vector>
#include <mutex>
#include <algorithm>
#include <csignal>
...
#include "zeprascript/wasm/WasmTrapHandler.h"
...
static void handleWasmFault(int signum, siginfo_t* info, void* context) {
    ...
    if (SignalHandlers::isJITAddress(pc)) {
        ...
        // Try to identify specific trap reason from metadata
        if (GlobalTrapRegistry::instance().lookupTrap(pc, reason)) {
            // Found exact trap site!
        } else {
            // Fallback: Guess based on fault address
            if (faultAddr == 0) {
